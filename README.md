...
otherwise. A live match clock (duration, pause, break) and courtside
...
the pitch. The MVP award is for fun and doesn't count toward it.
...
works well, see the install notes above). The app always proposes who plays next.
...
automatically before starting, so there is no manual migration step.
